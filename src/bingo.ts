 ...
 */
function makeBoardsList(boardsText: string): number[][][] {
 ...
 */
export function isWinner(board: number[][], calledNumbers: number[]): boolean {
...
function announceLastWinner(): void {
  const calledNumbers: number[] = fs.readFileSync(calledNumbersPath, 'utf8').split(',').map(Number);
  const boardsText: string = fs.readFileSync(boardsPath, 'utf8');
  const boards: number[][][] = makeBoardsList(boardsText);
  ...
  if (lastWinningBoardIndex !== -1) {
    console.log(
      `Board to pick against the giant squid: ${lastWinningBoardIndex + 1}. ` +
